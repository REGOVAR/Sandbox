...
class B006(PostgreBench):
	...
	# IMPORT VCF Data
	# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
	def import_vcf(file, records_count:int, out_log:str, out_stat:str):
	...
	# TEST Requests
	# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
	def test_req(out_log:str, out_stat:str):

		global connection, meta, session
		...
		with Timer() as t:
			result = connection.execute("SELECT COUNT(*) FROM _6_sample")
		...
		#count variant
		with Timer() as t:
			result = connection.execute("SELECT COUNT(*) FROM _6_variant")
		print("\nVariant total : " , result.first()[0], " (", t, ")")

		with Timer() as t:
			result = connection.execute("SELECT COUNT(DISTINCT(chr, pos, ref, alt)) FROM _6_variant")
		...
		with Timer() as t:
			result = connection.execute("SELECT sample_id, COUNT(*) FROM _6_variant GROUP BY sample_id")
		...
		with Timer() as t:
			result = connection.execute("SELECT pos, alt FROM _6_variant WHERE sample_id = 1 AND chr = '5' AND ref = 'A'")
		...
		with Timer() as t:
			result = connection.execute("SELECT chr, pos, ref, alt, count(sample_id ) as \"used\" FROM _6_variant GROUP BY chr, pos, ref, alt ORDER BY \"used\" DESC")
		...
		with Timer() as t:
			result = connection.execute("SELECT sample_id, is_transition, count(*) FROM _6_variant GROUP BY is_transition, sample_id ORDER BY sample_id, is_transition")
...
# Instanciate the bench class in the global dictionary
